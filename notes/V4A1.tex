...
\end{equation*}
%Here check how the addition of morphisms is defined in the preadditive categoreis and add the definition of zero objects and its existence
\end{definition}
...
\begin{definition}
%Write the definition of left/right exact functor
\end{definition}
...
\end{tikzcd}
\]
%Complete the proof for the equivalent characterization of splitting short exact sequence
\end{proof}
...
%consider writing the definition for complex 
...
\end{lemma}
\begin{proof}
%Proof for uniqueness of universal \delta functors with fixed H^0
\end{proof}
...
%Consider putting the definition of erasability in the category theory section.
\end{definition}
...
\begin{proof}
%See the exercise 13 for the universality of the erasable delta functors.
\end{proof}
...
%Refined the definition of the higher derived functors.
\end{definition}
...
\begin{lemma}
Let $\mathscr{A},\mathscr{B}$ be abelian categories with $\mathscr{A}$ having enough injectives, $F:\mathscr{A}\to\mathscr{B}$ be an additive left exact functor. \\
\par %Write the construction of boundary maps
\end{lemma}
\begin{proof}
%later
\end{proof}
...
\begin{lemma}
Let $\mathscr{A},\mathscr{B}$ be abelian categories with $\mathscr{A}$ having enough injectives, $F:\mathscr{A}\to\mathscr{B}$ be an additive left exact functor. \\
\par For any $i\in\mathbb{N}$, $R^iF$ is erasable.%Check the definition with functors
\end{lemma}
...
\begin{equation*}
\delta %Write the construction as in the previous lemma.
\end{equation*}
with $H^0=F$.
\end{corollary}
...
\begin{definition}
Let $\mathscr{A},\mathscr{B}$ be abelian categories with $\mathscr{A}$ having enough injectives, $F:\mathscr{A}\to\mathscr{B}$ be an additive left exact functor. \\ %Check if we can reduce the assumption on the definition of F-acyclic
...
\end{tikzcd}
\]
We can compute the cohomology by %Refine the definition of the cohomology with respect to F-acyclic resolutions.
\end{lemma}
...
\end{equation*}
is an additive left exact functor. %Need to prove the global section functor is an additive left exact functor
%Later
\end{example}

\begin{example}
Let $X,Y$ be topological spaces and $f:X\to Y$ be a continuous function. Then the direct image functor $f_*$ is an additive left exact functor. %Need to prove the direct image functor is an additive left exact functor
\end{example}
