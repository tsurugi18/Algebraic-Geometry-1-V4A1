...
\begin{remark}
Categorically, taking stalking is a functor for each $x\in X$. Suppose we have $\mathcal{F},\mathcal{G}\in\PreSh_\mathcal{A}(X)$ and a morphism $\varphi:\mathcal{F}\to\mathcal{G}$, then 
\begin{equation*}
\varphi_x:\mathcal{F}_x\to\mathcal{G}_x,\varphi_x(s_x)=\varphi(s)_x.
\end{equation*}

\[
\begin{tikzcd}
\mathcal{F}\arrow{r}{\varphi}\arrow{d}&\mathcal{G}\arrow{d}\\
\mathcal{F}_x\arrow{r}{\varphi_x}&\mathcal{G}_x
\end{tikzcd}
\]
\end{remark}
